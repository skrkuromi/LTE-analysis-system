...
class PRBInfoQuery extends React.Component {
    ...
    handleClickQuery = async () => {
        ...
        let sector = [];
        const result = await fetchTool('GET', '/tbKPI/query_by_enodeb_name', {
            enodeb_name: selectENodeB
        });
    ...
    dropdownAttrChange = async () => {
        const { eNodeBAttr } = this.state;

        if (eNodeBAttr.length === 0) {
            const result = await fetchTool('GET', '/tbKPI/allKPIInfo', {});
            if (result.status === undefined) {
                this.setState({
                    eNodeBAttr: result.msg
                })
            }
        }
    }
    ...
    render() {
        const { eNodeBName, data, eNodeBAttr } = this.state;
        const enodebOptions = eNodeBName.map(data => <Option key={data}>{data}</Option>);
        const attrOptions = eNodeBAttr.map(data => <Option key={data}>{data}</Option>);
